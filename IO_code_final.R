...
summary_holdings_and_sites <- summary_media_news %>% group_by(holding_or_site) %>% summarise(year_aud = sum(year_aud),
                                                                                             share = sum(share))
# топ 20
table_1 <- summary_holdings_and_sites %>% mutate(share = round(share*100, digits = 3)) %>% arrange(desc(share)) %>%
  select("Холдинг / Сайт" = holding_or_site, "Доля посещений" = share) %>% head(20)
...
# для индекса Ноама
n_1 <- summary_holdings_and_sites %>% filter(share*100 > 1) %>% summarise(n = n())
round(H_1 / (n_1)^0.5, digits = 3)
...
a_2 <- fed_or_gov %>% group_by(fed_or_ind) %>% summarise(share = sum(share) * 100)
table_2 <- a_2 %>% arrange(desc(share)) %>% head(20)
...
round(H_2 / (n_2)^0.5, digits = 3)

# сгруппируем по проправительственным компаниям
a_3 <- fed_or_gov %>% group_by(gov_or_ind) %>% summarise(share = sum(share) * 100)
table_3 <- a_3 %>% arrange(desc(share)) %>% head(20)
